...
dat.tmp.tem <- dat.tmp %>%
      group_by(region) %>%
      summarise(Total = sum(wasting > -1, na.rm = t), 
                wasting = sum(wasting, na.rm = t),) %>%
      mutate(Prop = wasting / Total) %>% as.data.frame()
...
```{r}
   ...
   clean_bino.dat.tmp<- bino.dat.tmp[complete.cases(bino.dat.tmp$wasting), ]
...
country <- "ZM"
survey <- "ZM2018DHS"
tab <- get_api_table(coutry, survey, "CN_NUTS_C_WH2")
tab
