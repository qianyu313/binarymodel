...
*dhs:PR-householdMember
*data process code: nt_ch_stunt "Stunted child under 5 years"
*api: binary_CH_VACS_C_DP3

Issue: no age in PRdata, no col of hb19
      no ByVariableLabel(under 60,...) in api
...
dat.tmp.tem <- dat.tmp %>%
      group_by(region) %>%
      summarise(Total = sum(stunting > -1, na.rm = t), 
                stunting = sum(stunting, na.rm = t),) %>%
      mutate(Prop = stunting / Total) %>% as.data.frame()
...
```{r}
   ...
   clean_bino.dat.tmp<- bino.dat.tmp[complete.cases(bino.dat.tmp$stunting), ]
   fit1 <- smoothSurvey(as.data.frame(clean_bino.dat.tmp),
            responseType =  "binary",
            responseVar= "stunting",
            regionVar = "region",
...
country <- "ZM"
survey <- "ZM2018DHS"
tab <- get_api_table(coutry, survey, "CN_NUTS_C_HA2")
tab
